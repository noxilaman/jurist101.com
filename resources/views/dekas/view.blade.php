                        ...
                        <div class="tab-content" id="myTabContent" style="background-color: #ffffff; overflow-y: auto;">
                            ...
                            {!! $lawdata->c_comment !!}
                                ...
                                <div class="tab-pane fade" id="nav-pang" role="tabpanel">
                                    <h3 class="mb-1">มาตรา</h3>
                                    <div id="accordion">
                                        @foreach ($lawdata->dekalawlinks as $sitem)
                                            <div class="card">
                                                <div class="card-header" data-toggle="collapse"
                                                    data-target="#collapse{{ $sitem->id }}" aria-expanded="false"
                                                    aria-controls="collapse{{ $sitem->id }}" id="heading{{ $sitem->id }}">
                                                    <h5 class="mb-0">
                                                        <button class="btn btn-link collapsed">
                                                            {{ $sitem->law->mainlaw->name }} {{ $sitem->law->c_name }}
                                                            {{ $sitem->law->short_name }}
                                                        </button>
                                                    </h5>
                                                </div>
                                                <div id="collapse{{ $sitem->id }}" class="collapse"
                                                    aria-labelledby="heading{{ $sitem->id }}" data-parent="#accordion">
                                                    <div class="card-body">
                                                        <div class="mB-10">
                                                            {!! $sitem->law->c_desc !!}
                                                        </div>
                                                        ...
                                                        <div class="mt-3 text-center">
                                                            <a class="btn btn-outline-secondary"
                                                                href="{{ url('/lawview/' . $sitem->law->i_id . '/' . htmlspecialchars(str_replace('/', '_', $sitem->law->c_name))) }}">อ่านเพิ่ม</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
